...
async function downloadLatestCompatibleChrome() {
    ...
    const matchedCompatibilityVersions = compatibilities.filter((cv) => {
        ...
        return (
            areVersionsCompatible(cv.pptr, puppeteerVersion) ||
            (puppeteerMajor >= cvMajor && puppeteerMajor - cvMajor <= 2)
        );
    });
    ...
    console.warn(
        `Attempting to find a Chrome installer for versions: ${compatibleChromeVersion.join(
            ', '
        )}`
    );

    const buffer = await downloadClosestChromeInstaller(
        compatibleChromeVersion
    );
...
(async () => {
    console.log(
        `Preparing to download the latest Chrome compatible with Puppeteer ${puppeteerVersion}`
    );
