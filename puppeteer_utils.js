...
const chromeVersionDownloadUrl = (version) =>
...
function parseCompatibilityVersions(text) {
    ...
    if (versions.length < 5) {
        throw new Error(
            'Cannot find and parse Puppeteer-Chromium versions from Markdown file.'
        );
    }
    return versions;
}
...
async function fetchCompatibilityVersions() {
    const apiResponse = await axios.get(
        'https://raw.githubusercontent.com/GoogleChrome/puppeteer/main/docs/api.md',
        { responseType: 'text' }
    );
...
async function downloadClosestChromeInstaller(versionToCheck) {
    ...
    const relevantParts = versionToCheck.map((item) =>
    ...
    do {
        const response = await axios.get(
            `https://www.ubuntuupdates.org/package/google_chrome/stable/main/base/google-chrome-stable?id=202706&page=${pageNumber}`,
            { responseType: 'text' }
        );
        ...
        for (const relevantPart of relevantParts) {
            ...
            if (
                !rawText.includes(relevantPart) ||
                versionsThatDoNotExistYet.has(relevantPart)
            ) {
                continue;
            }
            ...
            const foundVersion = rawMatches
                .filter((item) => item.startsWith(relevantPart))
                .sort()[0];

            console.log(
                `Attempting to download Chrome installer for version ${foundVersion}`
            );
            ...
            try {
                ...
                console.log(
                    `Downloaded version ${foundVersion} that is compatible with Puppeteer ${puppeteerVersion}`
                );
                debBuffer = res.data;
                break;
            } catch {
                // Ignore errors
                console.warn(
                    `Couldn't find or download Chrome stable with version ${foundVersion}`
                );
